...
public class PhoenixSkill extends Skill {
    ...
    public MutableText getTooltipText(int level) {
        return Text.translatable(getTranslateKey() + ".tooltip",
                Text.literal(Utils.decimal("#", getInt("time", level) / 20.0)).formatted(Formatting.GOLD), Text.literal(Utils.decimal(getDouble("time", level) / 20)).formatted(Formatting.GOLD), Text.literal(Utils.decimal(get("mp", level))).formatted(Formatting.GOLD));
    }
}
